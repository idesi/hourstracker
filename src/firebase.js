...
const getOneByValue = ref => (valueKey, value) => ref.orderByChild(valueKey).limitToLast(1).once('value');
...
const timeEntryPush = async data => {
  ...
  let existingData = await getOneByValue(timeEntryRef)('date_userID', data.date_userID);

  //We get back key : { data... }
  //By calling Object.values we get an array [0] : { data... }
  existingData = Object.values(existingData.val())[0];
